...
// - Emits ONLY chat lines to the browser: {type:"chat", who:"User"|"Agent", text}
// - Ignores client text; forwards only PCM binary upstream
  ...
  wss.on("connection", (client, req) => {
    ...
    function sendSettings() {
      ...
      const settings = {
        ...
        // You can keep output PCM if you want to play locally
    ...
    // Emit a clean chat message to browser
    function sendChat(who, text) {
      if (!text) return;
      try { client.send(JSON.stringify({ type: "chat", who, text })); } catch {}
    }
      ...
      if (!isBuf || (isBuf && data.length && data[0] === 0x7b)) {
        ...
        switch (evt.type) {
          ...
          case "ConversationText": {
            ...
            const txt = String(evt.content ?? evt.text ?? "").trim();
            if (!txt) break;
            const who = roleRaw.includes("agent") || roleRaw.includes("assistant") ? "Agent" : "User";
            sendChat(who, txt);
            break;
          }

          case "UserResponse": {
            const txt = String(evt.content ?? evt.text ?? evt.transcript ?? "").trim();
            if (txt) sendChat("User", txt);
            break;
          }
        ...
        // NOTE: we intentionally DO NOT mirror full JSON to the browser anymore.
        return;
      }
    ...
    client.on("message", (msg, isBinary) => {
      ...
      if (!isBinary && typeof msg === "string") {
        // swallow any client JSON control
        return;
      }
